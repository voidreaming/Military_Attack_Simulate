...
//             precomputedPaths[fighter.id][i] = path;
//         }
//     }
// }
...
void simulate() {
    ...
    for (int frame_number = 0; frame_number < total_frames; frame_number++) {
        ...
        for (auto& fighter : fighters) {
            ...
            // cerr << "filghter "<<fighter.id << " missiles: "<< fighter.missiles << ";  fuel: "<< fighter.fuel <<endl;
            ...
            for (int i = 0; i < red_bases.size(); ++i) {
                auto& base = red_bases[i];
                if (base.defense > 0) { // Only consider bases that still need to be destroyed
                    auto path = bfsPath(sx, sy, base.x, base.y);
                    
                    if (!path.empty() && path.size() < min_dist) {
                        min_dist = path.size();
                        closest_base = i;
                        best_path = path;
                        // cerr << "closest base to " << fighter.id << "  is (" << base.x << "," << base.y << ");" << endl;
                    }
                }
            ...
            //     if (fighter.fuel > 0 && fighter.missiles >= red_bases[base_id].defense && calculateDistance(fighter.x, fighter.y, red_bases[base_id].x, red_bases[base_id].y) <= fighter.range) {
            //         attackBase(fighter, red_bases[base_id], getDirection(fighter.x, fighter.y, red_bases[base_id].x, red_bases[base_id].y));
            //     }
            // }

            //if there is a reachable base, decide move towards it and attack
            if (closest_base != -1 && !best_path.empty()) { 
                auto& base = red_bases[closest_base];
                // Move towards to closest base,  Generate move commands along the path
                
                moveFighter(fighter, getDirection(best_path[0].first, best_path[0].second, best_path[1].first, best_path[1].second));
                // for (size_t i = 1; i < best_path.size() && fighter.fuel > 0 && !fighter.actionTaken; ++i) {
                //     moveFighter(fighter, getDirection(best_path[i-1].first, best_path[i-1].second, best_path[i].first, best_path[i].second));
                // }
                
                //Attack after reach the attack range
                if (fighter.fuel > 0 && fighter.missiles >= base.defense && calculateDistance(fighter.x, fighter.y, base.x, base.y) <= fighter.range) {
                attackBase(fighter, base, getDirection(fighter.x, fighter.y, base.x, base.y));
                }

                
            }
            cerr << "fuel remained : " << fighter.id << fighter.fuel << endl;
        }
